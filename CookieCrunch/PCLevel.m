...
- (instancetype)initWithFile:(NSString *)filename {
  ...
  if (self != nil) {
    ...
    [dictionary[@"tiles"] enumerateObjectsUsingBlock:^(NSArray *array, NSUInteger row, BOOL *stop) {
      ...
      [array enumerateObjectsUsingBlock:^(NSNumber *value, NSUInteger column, BOOL *stop) {
        ...
        if ([value integerValue] == 1) {
          _tiles[column][tileRow] = [[PCTile alloc] init];
        }
      }];
    }];
    ...
    self.maximumMoves = self.maximumMoves + purchasedMoves;
  ...
  for (NSInteger row = 0; row < NumRows; row++) {
    for (NSInteger column = 0; column < NumColumns; column++) {
      ...
      if (_tiles[column][row] != nil) {
        ...
        PCJelly *cookie = [self createCookieAtColumn:column row:row withType:cookieType];
...
- (PCJelly *)createCookieAtColumn:(NSInteger)column row:(NSInteger)row withType:(NSUInteger)cookieType {
  PCJelly *cookie = [[PCJelly alloc] init];
...
- (void)detectPossibleSwaps {
  ...
  for (NSInteger row = 0; row < NumRows; row++) {
    for (NSInteger column = 0; column < NumColumns; column++) {
      ...
      PCJelly *cookie = _cookies[column][row];
      if (cookie != nil) {
        ...
        if (column < NumColumns - 1) {
          ...
          PCJelly *other = _cookies[column + 1][row];
          if (other != nil) {
            // Swap them
        ...
        if (row < NumRows - 1) {
          ...
          // Have a cookie in this spot? If there is no tile, there is no cookie.
          PCJelly *other = _cookies[column][row + 1];
          if (other != nil) {
            // Swap them
            _cookies[column][row] = other;
      ...
      if (_cookies[column][row] != nil) {
        ...
        if (_cookies[column + 1][row].cookieType == matchType
         && _cookies[column + 2][row].cookieType == matchType) {
          ...
          NSInteger lastColumnThatMatches=-1;
          ...
          do {
            ...
            lastColumnThatMatches = column;
            
            column += 1;
              
              
          }//do it while not at the last collumn and the type is the same
          while (column < NumColumns && _cookies[column][row].cookieType == matchType);

            /*
            NSSet *lOrTShapesAtCol1 = [self detectVerticalTLMatchesForColumn: column andStartRow:row ofType:matchType];
            
            NSSet *lOrTShapesAtCol2 = [self detectVerticalTLMatchesForColumn: lastColumnThatMatches andStartRow:row ofType:matchType];
            
            bool hasRowUp = false;
            bool hasRowDown = false;
            
            for(RWTCookie *cookie in lOrTShapesAtCol1) {
                //NSInteger theCol = cookie.column;
                NSInteger theRow = cookie.row;
                if(theRow < row) {
                    hasRowUp = true;
                }
                if(theRow > row) {
            ...
            if(hasRowDown && hasRowUp) {
                chain.chainType = ChainTypeT;
            }
            else if(hasRowUp || hasRowDown) {
                chain.chainType = ChainTypeL;
            }
            //---------------------------------------
            hasRowUp = false;
            hasRowDown = false;
            
            for(RWTCookie *cookie in lOrTShapesAtCol2) {
                //NSInteger theCol = cookie.column;
                NSInteger theRow = cookie.row;
                if(theRow < row) {
                    hasRowUp = true;
...
- (NSSet *)detectVerticalTLMatchesForColumn:(NSInteger) column andStartRow:(NSInteger) startRow ofType:(NSUInteger) type{
    
    NSMutableSet *set = [NSMutableSet set];
    
    //first go down, increasing the row until the last one
        for (NSInteger row = startRow+1; row < NumRows ; row++) {
            //if we have a cookie there
            if (_cookies[column][row] != nil) {
                //and is of same type
                if (_cookies[column][row].cookieType == type) {
                    PCJelly *cookieToAddInChain = _cookies[column][row];
                    [set addObject:cookieToAddInChain];
                }
            }
        }
    
    //then go up, decreasing the row until the last one
    for (NSInteger row = startRow-1; row >= 0 ; row--) {
        //if we have a cookie there
        if (_cookies[column][row] != nil) {
            //and is of same type
            if (_cookies[column][row].cookieType == type) {
    ...
    for (NSInteger row = 0; row < NumRows; row++) {
      ...
      if (_tiles[column][row] != nil && _cookies[column][row] == nil) {
        ...
        for (NSInteger lookup = row + 1; lookup < NumRows; lookup++) {
          PCJelly *cookie = _cookies[column][lookup];
          if (cookie != nil) {
...
- (PCTile *)tileAtColumn:(NSInteger)column row:(NSInteger)row {
...
- (PCJelly *)cookieAtColumn:(NSInteger)column row:(NSInteger)row {
  NSAssert1(column >= 0 && column < NumColumns, @"Invalid column: %ld", (long)column);
  NSAssert1(row >= 0 && row < NumRows, @"Invalid row: %ld", (long)row);
...
- (BOOL)isPossibleSwap:(PCSwap *)swap {
